...
> tips: 还有一种ex mode.这种模式支持多次键入cmd. 它不支持map 和abbrev. 参阅`:h Q`
...
## Ctrl+Char 问题
vim 对Ctrl+Char的支持其实非常有限: <C-char>对应的keycode就是 char & 0x1F, 它只能表示31个keycode.(这个char有一个范围,不能是数字, 只能是C0 Control_character) <C-a> 与 <C-A> 对vim而言都视为是0x01 (因为 0x41 & 0x1f == 0x61 & 0x1f). 参阅[<C-char> map in vim](http://superuser.com/questions/580110/why-does-vim-require-every-key-mapping-to-have-a-unique-ascii-code). 
[Control Character](http://zh.wikipedia.org/wiki/%E6%8E%A7%E5%88%B6%E5%AD%97%E7%AC%A6)

## Ctrl+s/Ctrl+q 问题
终端默认会拦截Ctrl+s/Ctrl+q, 它们属于终于的控制符。你可以从这里获得更多： `stty -a` and  `man stty`.
...
此时，如果终端需要绑定这两个按键，可以关闭它们(在.zshrc 或者 .bashrc 中):
